...
Crawler.prototype.makeQueue = function(worker) {
  ...
  var queue = async.queue(function crawl(link, next) {
      ...
      request(link, function(err, response, body){
          ...
          // output some details
          //for(var i in queue.tasks) {
          //    console.log("pending", queue.tasks[i].data)
          //}

          //plug in your worker here
          self.worker.work(body, self.data, function(links){
              console.log("Pending %d, Current %s, Found %j", queue.tasks.length, link, links);
