...
Crawler.prototype = {
    "seen"     : [],
    "storage"  : {},
    "badLinks" : /\.(bmp|exe|jpeg|swf|pdf|gif|png|jpg|doc|avi|mov|mpg|tiff|zip|tgz|xml|rss|mp3|ogg|wav|rar)$/i,
    "crawling" : true
};
 ...
 * @returns async.queue
 */
Crawler.prototype.makeQueue = function(uri, jobQueue, crawler, job) {
    ...
    queue.drain = function() {                                             // Adds our drained
        console.log('all items processed\nFound %j', self.getStore());
        jobQueue.deleteJob(job.id, self);
    }
}
